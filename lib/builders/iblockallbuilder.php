...
/**
 * Делает миграции для всего ИБ
 * 3 штуки: Структура, разделы, элементы
 */
class IblockallBuilder extends IblockBuilder
{
    protected function initialize()
    {
        $this->setTitle(Locale::getMessage('BUILDER_IblockallExport_Title'));
        $this->setDescription(Locale::getMessage('BUILDER_IblockallExport_Descr'));
    ...
    protected function execute()
    {
        ...
        $basePrefix = $this->getFieldValue('prefix');
        $versionManager = new VersionManager();

        if (!isset($this->params['total'])) {
            //миграция структуры ИБ
            $this->bindField(
                'prefix',
                $basePrefix . '_structure'
            );
            ...
            //Генерируем миграцию для разделов
            $this->unbindField('prefix');
            $this->bindField(
                'prefix',
                $basePrefix . '_sections'
            );
            $sectionsVersionName = $this->createVersionName();
            $versionTimeStamp = $versionManager->getVersionTimestamp($sectionsVersionName);
            $this->params['~version_name'] = str_replace(
                $versionTimeStamp,
                $versionTimeStamp + 1,
                $sectionsVersionName
            );
        ...
        //Генерируем миграцию для элементов
        if (strpos($basePrefix, '_elements') === false) {
            $this->unbindField('prefix');
            $this->bindField(
                'prefix',
                $basePrefix . '_elements'
            );
            $elementsVersionName = $this->createVersionName();
            $versionTimeStamp = $versionManager->getVersionTimestamp($elementsVersionName);
            $this->params['~version_name'] = str_replace(
                $versionTimeStamp,
                $versionTimeStamp + 2,
                $elementsVersionName
        ...
        $exportFilter = array();
        $exportFields = array_keys($iblockFields);
        $exportProps = array_column($iblockProperties, 'CODE');
