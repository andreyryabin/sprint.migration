...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["step_code"] == "migration_new" && check_bitrix_sessid('send_sessid')) {
    ...
    if ($manager->getOption('admin_versions_view') != 'new'){
        $manager->setOption('admin_versions_view', 'new');
    }
    ...
    <?if (!empty($versions)): ?>
        ...
        <?foreach ($versions as $aItem): ?>
            <div class="c-migration-block">
                <a href="#" onclick="migrationMigrationDescr('<?= $aItem['version'] ?>');return false;" class="c-migration-item-is_new"><?= $aItem['version'] ?></a>
                &nbsp;
